...
export default function DoctorDashboardView({ patients, currentUser, onOpenOverride, onOpenReport }) {
  ...
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', borderRadius: '16px', padding: '20px', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: '#94a3b8' }}>
            Logged in as: <strong style={{ color: '#00d4ff' }}>{currentUser ? currentUser.name : 'Dr. Sarah Jenkins, MD'}</strong>
          </p>
        </div>

        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div style={{ background: 'rgba(0,150,199,0.15)', border: '1px solid #0096c7', borderRadius: '10px', padding: '8px 14px', fontSize: '0.82rem', fontWeight: 700, color: '#00d4ff' }}>
            🏥 Active Queue: {patients.length} Patients
          </div>
        </div>
      </div>

      {/* Grid: Left Patient Selector / Right Clinical Detail & Note Entry */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '20px' }}>
        ...
        {/* Right Patient Profile & Diagnosis Writer */}
        {selectedPatient ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ background: '#f0f9ff', border: '1.5px solid #7dd3fc', borderRadius: '14px', padding: '20px' }}>
              <h3 style={{ margin: '0 0 12px', fontSize: '1rem', fontWeight: 800, color: '#0369a1' }}>
                📝 Add Doctor Consultation Note / Prescription / Diagnosis
              </h3>
              ...
              <form onSubmit={handleAddNote} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  ...
                  <div>
                    <label style={{ fontSize: '0.78rem', fontWeight: 700 }}>Title / Subject</label>
                    <input
                      type="text"
                      className="input-field"
                      value={noteTitle}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="3"
                    placeholder="Enter clinical examination notes, diagnosis, medication dosages, and follow-up instructions..."
                    value={noteContent}
                ...
                <button type="submit" className="btn btn-primary" disabled={savingNote}>
                  {savingNote ? 'Saving Note...' : '💾 Save Consultation Note'}
                </button>
              </form>
            </div>
            ...
            <div style={{ background: '#ffffff', border: '1px solid var(--border-color)', borderRadius: '14px', padding: '20px' }}>
              <h3 style={{ margin: '0 0 14px', fontSize: '1rem', fontWeight: 800, color: '#0f172a' }}>
                📜 Complete Patient History Timeline ({formattedSelectedPatId})
              </h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {timeline.length === 0 ? (
                  ...
                  timeline.map(evt => (
                    <div key={evt.id} style={{ borderLeft: '3px solid #0096c7', paddingLeft: '14px', position: 'relative' }}>
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Select a patient from the left queue.</div>
        )}
